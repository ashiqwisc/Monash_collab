...
```{r, message = FALSE}
df_location <- read.csv("data/with_comparison.csv") %>%
  filter(modality == "location") %>%
  as.data.frame()

df_dialogue <- read.csv("data/with_comparison.csv") %>%
...
location_code_cols <- c("collaborate_primary", "independent_primary", "collaborate_secondary", "independent_secondary", "task_distribution", "task_transition")
