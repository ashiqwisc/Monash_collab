...
# df <- read_excel("~/Desktop/epistemic_analytics/Monash_collab/data/merged_A_15_7_8_codes_with_conversation_id.xlsx") %>% as.data.frame()
...
# # Two new codes, not sure what they stand for
# colnames(df)[which(names(df) == "id")] <- "ID"
# colnames(df)[which(names(df) == "cd")] <- "CD"
# 
# df <- df[, -c(1)]
# write.csv(df, "~/Desktop/epistemic_analytics/Monash_collab/data/with_comparison.csv", row.names = FALSE)
# View(df)
```

```{r, message = FALSE}

df <- read_csv("~/Desktop/epistemic_analytics/Monash_collab/data/with_comparison.csv") %>% as.data.frame()
# df
```
...
unit_cols <- c("session_id", "initiator", "comparison") 
...
code_cols <- c("collaborate_primary", "independent_primary", "collaborate_secondary", "independent_secondary", "task_distribution", "task_transition", "ID", "CD", "task_allo_and_plann", "provision_of_handover_information","info_sharing_and_situ_assess", "escalation", "information_requesting", "responding_to_request", "agreement")
...
HOO_rules_model <-  tma:::rules(
    session_id %in% UNIT$session_id & conversation_id %in% UNIT$conversation_id
)
```
